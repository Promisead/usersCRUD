...
exports.create = (req, res) => {
  ...
  user
    ...
    .catch((err) => {
      res.status(500).send({
        message:
          err.message || "Some error occurred while creating an operation",
      });
    });
};
...
//Delete a user with spaecified user id in the request
exports.delete = (req, res) => {
  ...
  Userdb.findByIdAndDelete(id)
    .then((data) => {
      if (!data) {
        res.status(404).send({
          message: `Cannot delete user with ${id}. Maybe user id incorrect `,
        });
      } else {
        res.send({
